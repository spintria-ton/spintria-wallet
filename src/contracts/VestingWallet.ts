...
import { endParse } from './VestingMinter';
...
export class VestingWallet implements Contract {
  ...
  async sendTransfer(
    ...
    await provider.internal(via, {
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: VestingWallet.transferMessage(
        jetton_amount,
        to,
        responseAddress,
        customPayload,
  ...
  async sendBurn(
    ...
    await provider.internal(via, {
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: VestingWallet.burnMessage(jetton_amount, responseAddress, customPayload),
      value: value,
    });
  }
  /*
  ...
  async sendWithdrawTons(provider: ContractProvider, via: Sender) {
    await provider.internal(via, {
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: VestingWallet.withdrawTonsMessage(),
      value: toNano('0.1'),
    });
  }
  /*
  ...
  async sendWithdrawJettons(
    ...
    await provider.internal(via, {
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: VestingWallet.withdrawJettonsMessage(from, amount),
      value: toNano('0.1'),
    });
  }

  /*
      clawback#b5de5f9e query_id:uint64 custom_payload:(Maybe ^Cell) = InternalMsgBody;
    */
  static clawbackMessage(customPayload: Cell | null) {
    return beginCell()
      .storeUint(Op.clawback, 32)
      .storeUint(0, 64) // op, queryId
      .storeMaybeRef(customPayload)
      .endCell();
  }
